#!/usr/bin/env node
/**
 * Remove JPG objects from Supabase Storage and update DB URLs (.jpg/.jpeg → .webp).
 * Usage: node scripts/migrate-remote-jpg-to-webp.mjs
 */
...
function replaceJpgInValue(value) {
  if (typeof value === "string") {
    return /\.jpe?g\b/i.test(value) ? value.replace(/\.jpe?g\b/gi, ".webp") : value;
  }
  if (Array.isArray(value)) return value.map(replaceJpgInValue);
  if (value && typeof value === "object") {
    const out = {};
    for (const [k, v] of Object.entries(value)) out[k] = replaceJpgInValue(v);
    return out;
  }
  return value;
}
...
async function uploadWebpFromMapping() {
  const mappingPath = resolve(root, "scripts/.jpg-to-webp-mapping.json");
...
async function deleteJpgFromStorage() {
  const [pngRes, jpgRes] = await Promise.all([
    admin.from("media_assets").select("id,folder,filename").ilike("filename", "%.jpg"),
    admin.from("media_assets").select("id,folder,filename").ilike("filename", "%.jpeg"),
  ]);
  if (pngRes.error) throw new Error(pngRes.error.message);
  if (jpgRes.error) throw new Error(jpgRes.error.message);

  const rows = [...(pngRes.data ?? []), ...(jpgRes.data ?? [])];
...
async function updateTableJson(table, idCol, rows, jsonCols) {
  ...
  for (const row of rows) {
    ...
    for (const col of jsonCols) {
      ...
      const next = replaceJpgInValue(row[col]);
    ...
    for (const col of ["hero_image_url", "image_url", "desktop_image_url", "mobile_image_url", "url"]) {
      if (typeof row[col] === "string" && /\.jpe?g\b/i.test(row[col])) {
        patch[col] = row[col].replace(/\.jpe?g\b/gi, ".webp");
        changed = true;
      }
    }
    if (!changed) continue;
...
async function main() {
  ...
  const [vehicles, accessories, banners, mediaAssets, cmsPages] = await Promise.all([
  ...
  const c = await updateTableJson("cms_pages", "slug", cmsPages.data ?? [], ["content"]);

  console.log(`✅ vehicles: ${v}, accessories: ${a}, banners: ${b}, media_assets: ${m}, cms_pages: ${c}\n`);

  console.log("→ Xóa JPG khỏi Storage + media_assets...");
  const deleted = await deleteJpgFromStorage();
  console.log(`✅ Removed ${deleted} JPG objects\n`);
