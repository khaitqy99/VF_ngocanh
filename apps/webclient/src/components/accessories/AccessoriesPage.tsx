...
import { ACCESSORY_HERO_BANNERS, type HeroBannerSlide } from "@/lib/images";
import {
      ...
      {adminEdit ? (
        <div className="sticky top-0 z-30 border-b border-brand/30 bg-brand-dark px-4 py-2.5 text-white shadow-lg">
          <p className="container-vf text-center text-xs font-semibold">
            Sửa thẻ catalog trực tiếp — danh mục, tên, mô tả, giá, badge & ảnh trên từng card
          </p>
        </div>
      ) : null}
      <main>
                  ...
                  >
                    {filteredProducts.map((product, index) => (
                      <CatalogGridItem key={product.id} index={index}>
                        <AccessoryProductCard product={product} adminEdit={adminEdit} />
                      </CatalogGridItem>
                    ))}
                  </CatalogGrid>
                )}
      ...
      <div className="container-vf grid gap-6 sm:grid-cols-2">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h3 className={`${vfPanelTitle} text-brand-dark`}>BẢO HÀNH CHÍNH HÃNG 6 - 24 THÁNG</h3>
  ...
  return (
    <div className="page-section-card max-h-[min(70vh,640px)] overflow-y-auto p-5 lg:max-h-[calc(100vh-10rem)] lg:overflow-y-auto">
      ...
      <div className="space-y-5">
        ...
        <FilterGroup title="DÒNG XE HỖ TRỢ" defaultOpen={false}>
