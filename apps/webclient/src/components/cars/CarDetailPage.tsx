...
export default function CarDetailPage({
  ...
  const prevImage = () => setActiveImage((i) => (i === 0 ? heroImages.length - 1 : i - 1));
  const nextImage = () => setActiveImage((i) => (i === heroImages.length - 1 ? 0 : i + 1));
  ...
  useEffect(() => {
    if (heroImages.length && activeImage >= heroImages.length) {
      setActiveImage(0);
    }
  }, [heroImages.length, activeImage]);
              ...
              {related.map((car, i) => (
                ...
                >
                  <CarCatalogCard
                    ...
                    onEstimatePrice={() => router.push(`${carDetailPath(car)}#tai-chinh`)}
                  />
                </motion.div>
              ))}
            </motion.div>
...
function OverviewSection({
  ...
  const overviewImage =
    detail.overview.image !== "/images/cars/oto-hero.webp"
      ? detail.overview.image
      : (detail.gallery[0] ?? detail.overview.image);

  const bullets = adminEditable ? [] : detail.overview.bullets;
    ...
    <>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <InlineSectionTitle
          ...
          subtitleFallback="Phong cách mạnh mẽ, sang trọng"
          adminEditable={adminEditable}
        />
      </div>
      <EditableFeatureGridSection
...
function InteriorSection({
  ...
  return (
    <>
      <InlineSectionTitle
        titlePath="_section.interior.title"
        titleFallback="Nội thất"
        subtitlePath="_section.interior.subtitle"
        subtitleFallback="Không gian cabin cao cấp, tiện nghi vượt trội"
        adminEditable={adminEditable}
      />
      <EditableFeatureGridSection
        path="interior"
        items={detail.interior}
        adminEditable={adminEditable}
        fallbackImage={detail.image}
        labels={INTERIOR_GRID_LABELS}
        expandToGrid={expandFeatureItemsForGrid}
      />
    </>
  );
      ...
      <PdpSectionTitle
        ...
        subtitle={
          <InlineText
            ...
            fallback={detail.technologySubtitle ?? "Hệ sinh thái kết nối toàn diện"}
            adminEditable={adminEditable}
    ...
    <>
      ...
      <PdpPerformanceShowcase
        ...
        metrics={mapEditableMetrics(
          buildPerformanceMetrics(detail.quickSpecs),
          "_perfMetrics",
          adminEditable,
        )}
...
function PrivilegesSection({
  ...
  privileges: PrivilegesSectionData;
  adminEditable?: boolean;
}) {
  const features = privileges.features.slice(0, 4);
  ...
  return (
    <>
      <PdpSectionTitle
        ...
        subtitle={
          <InlineText
            path="privileges.subtitle"
            fallback={privileges.subtitle}
            adminEditable={adminEditable}
            className="text-sm text-muted-foreground sm:text-[15px]"
            multiline
          />
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {editableHighlights(
          ...
          "privileges.highlights",
          adminEditable,
          "rounded-full border border-amber-300/40 bg-amber-50 px-3 py-1 text-[11px] font-semibold text-amber-900",
        )}
      </div>
      {adminEditable ? (
        <EditableFeatureGridSection
          path="privileges.features"
          ...
          adminEditable={adminEditable}
          fallbackImage={privileges.image}
        />
      ) : (
        <PdpImageFeatureGrid
          items={features.map((f) => ({
...
function SafetySection({ detail, adminEditable }: { detail: CarDetail; adminEditable?: boolean }) {
  const safetyImage =
...
function SpecsSection({ detail, adminEditable }: { detail: CarDetail; adminEditable?: boolean }) {
  const edit = useAdminEdit();
...
function ReviewsSection({ detail, adminEditable }: { detail: CarDetail; adminEditable?: boolean }) {
...
function SectionHeader({
  ...
  return (
    <div className={center ? "mx-auto max-w-3xl text-center" : ""}>
      <h2 className={`${vfSectionHeadingLeft} ${light ? "text-white" : ""}`}>{title}</h2>
      {subtitle && (
        <p
