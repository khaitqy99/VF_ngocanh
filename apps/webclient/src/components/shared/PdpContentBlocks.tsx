...
export function PdpQuickSpecBar({ specs, embedded }: { specs: PdpSpecItem[]; embedded?: boolean }) {
  ...
  return (
    <FadeIn>
      <div className="border-y border-border/40 bg-surface-muted">
            ...
            (bullets.length > 0 && (
              <ul className="mt-8 grid gap-3 sm:grid-cols-2">
                {bullets.map((bullet, i) => (
                  <li
                    key={`bullet-${i}`}
                    className="page-section-card flex items-start gap-3 px-4 py-3.5"
                  >
    ...
    <FadeIn>
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        {modes.map((mode, i) => (
          <div
            key={`drive-mode-${i}`}
            className="rounded-xl border border-border/50 bg-surface-muted px-4 py-4 text-center"
          >
            ...
            <div className="border-t border-border/40 p-6 sm:p-8 lg:col-span-7 lg:border-t-0 lg:border-l lg:p-10">
              ...
              {featuresSlot ? (
                ...
                <ul className="mt-6 grid gap-3 sm:grid-cols-2">
                  {features?.map((f, i) => (
                    <li key={`safety-feat-${i}`} className="page-section-card flex gap-3 p-4">
