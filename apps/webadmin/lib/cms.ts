...
async function fetchDraftProductStats(): Promise<{
  draftCarCount: number;
  draftScooterCount: number;
  draftAccessoryCount: number;
  draftMissingImageCount: number;
  ...
  } = await import("@/lib/product-publish-check");

  try {
    const admin = createAdminClient();
    const [cars, scooters, accessories] = await Promise.all([
      admin.from("vehicles").select("*").eq("type", "car").eq("status", "draft"),
      admin.from("vehicles").select("*").eq("type", "scooter").eq("status", "draft"),
      admin.from("accessories").select("*").eq("status", "draft"),
    ]);

    const draftCars = cars.data ?? [];
    const draftScooters = scooters.data ?? [];
    const draftAccessories = accessories.data ?? [];
    ...
    return {
      ...
      draftMissingImageCount: missingImages,
    };
  } catch {
    return {
      draftCarCount: 0,
...
export async function getAdminDashboardStats(): Promise<AdminDashboardStats> {
  ...
  const [cars, scooters, accessories, draftStats] = await Promise.all([
    getAdminCars(),
    getAdminScooters(),
    getAdminAccessories(),
    fetchDraftProductStats(),
  ]);
  return {
